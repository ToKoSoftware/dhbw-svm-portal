    ...
    try {
        ...
        const eventData: EventData | null = await Event
            ...
            .findOne({
                ...
                } : { include: { model: User.scope('publicData'), as: 'author' } }
            })
            .catch(() => {
                success = false;
                return null;
...
export async function getEvents(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
        let success = true;
