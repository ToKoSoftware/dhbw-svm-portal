...
export async function updateEvent(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
        let success = true;
        ...
        if (
            ...
            return res.status(403).send(wrapResponse(false, {
                error: 'You are not allowed to update an Event for a team you are not maintainer of.'
            }));
        }
