...
export async function createEvent(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
        ...
        if (!maintainedTeamIds.find(id => id == mappedIncomingData.allowed_team_id) &&
            mappedIncomingData.allowed_team_id !== 'public' && !Vars.currentUserIsAdmin) {
            return res.status(403).send(wrapResponse(false, {
                error: 'You are not allowed to create an Event for a team you are not maintainer of.'
            }));
        }
