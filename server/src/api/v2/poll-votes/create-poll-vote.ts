...
export async function voteForPollAnswer(req: Request, res: Response): Promise<Response> {
    ...
    const pollData: Poll | null = await Poll
        .scope([{ method: ['onlyCurrentOrg', Vars.currentOrganization.id] }, { method: ['onlyAnswerTeam', Vars.currentUser.teams.map(t => t.id), Vars.currentOrganization.public_team_id] }, 'active', { method: ['notExpired', currentDate] }])
